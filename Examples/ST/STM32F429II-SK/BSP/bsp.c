...
#define BSP_GPIOB_LED1	DEF_BIT_15
#define BSP_GPIOB_LED2	DEF_BIT_13
#define BSP_GPIOB_LED3	DEF_BIT_12
#define BSP_BUZZER 		DEF_BIT_07
...
static void  BSP_LED_Init()
{
	   ...
	   led_init.GPIO_Pin    = GPIO_Pin_15 | GPIO_Pin_13 | GPIO_Pin_12;
...
static void BSP_BUZZER_Init()
{
    ...
    buzzer_init.GPIO_Pin = GPIO_Pin_7;

    GPIO_Init(GPIOC, &buzzer_init);
}

void BSP_BUZZER_ON()
{
	GPIO_ResetBits(GPIOC, BSP_BUZZER);
...
void BSP_BUZZER_OFF()
{
	GPIO_SetBits(GPIOC, BSP_BUZZER);
}


void BSP_SENSOR_Init(){

	ADC_InitTypeDef fireSen_init = {0};
	GPIO_InitTypeDef fireSenGPIO_init = {0};

	RCC_APB2PeriphClockCmd(RCC_APB2Periph_ADC1, ENABLE); //The ADC1 is connected the APB2 peripheral bus thus we will use its clock source
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOC, ENABLE);

    fireSenGPIO_init.GPIO_Mode = GPIO_Mode_AN;
    fireSenGPIO_init.GPIO_PuPd = GPIO_PuPd_NOPULL;
    fireSenGPIO_init.GPIO_Pin = GPIO_Pin_0;

    ADC_DeInit();

    fireSen_init.ADC_DataAlign = ADC_DataAlign_Right;
    fireSen_init.ADC_Resolution = ADC_Resolution_12b;
    fireSen_init.ADC_ContinuousConvMode = ENABLE;
    fireSen_init.ADC_ExternalTrigConv = ADC_ExternalTrigConv_T1_CC1;
    fireSen_init.ADC_ExternalTrigConvEdge = ADC_ExternalTrigConvEdge_None;
    fireSen_init.ADC_NbrOfConversion = 1;
    fireSen_init.ADC_ScanConvMode = DISABLE;

   	GPIO_Init(GPIOC, &fireSenGPIO_init);
   	ADC_Init(ADC1, &fireSen_init);
   	ADC_Cmd(ADC1, ENABLE);
